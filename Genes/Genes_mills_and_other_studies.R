...
system("~/poverlap/poverlap2.py poverlap --a ../../other_studies/all_intervals_mouse_studies_10mb.txt --b reduced_intervals_our_study_10mb.txt -g ../../mouse.mm10.genome --n 9999")
system("bedtools intersect -wo -a ../../other_studies/all_intervals_mouse_studies_10mb.txt -b reduced_intervals_our_study_10mb.txt > bed_reduced_intersect_out.txt")
...
write_delim(final_out, "intervals_in_other_studies_with_genes_SW.csv", delim=";")

genes_list_SW_in_overlaps<- out %>% 
  distinct(GENESYMBOL)

write.table(genes_list_SW_in_overlaps, file="genes_intervals_in_other_studies-SW.txt", row.names = F, quote = F, col.names = F)
...
write(all_marker_genes, "../../all_marker_genes.txt")
...
write_csv(markers_in_overlap, "markers_in_other_studies_SW.csv")
...
write(genes_intervals_in_mills$mills, "genes_intervals_in_mills.txt")
# which genes from snps are in Mills ####
closest_genes_snps_in_mills <- all_mills_genes %>% 
  filter(mills %in% stringr::str_to_upper(all_marker_genes$V1))
write(closest_genes_snps_in_mills$mills, "closest_genes_snps_in_mills.txt")
# which genes from intervals are in other studies and Mills ####
genes_intervals_in_mills_and_other_studies <- all_mills_genes %>% 
  filter(mills %in% stringr::str_to_upper(genes_list_SW_in_overlaps$V1)) %>% drop_na() 
write(genes_intervals_in_mills_and_other_studies$mills, "genes_intervals_in_mills_and_other_studies.txt")
# which closest genes snps are in other studies and Mills ####
closest_genes_snps_mills_and_other_studies <- all_mills_genes %>% 
  filter(mills %in% stringr::str_to_upper(marker_genes_in_other_studies$V1))
write(closest_genes_snps_mills_and_other_studies$mills, "closest_genes_snps_in_mills_and_other_studies.txt")
