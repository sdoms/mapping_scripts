...
SV <- colnames(pheno)[tx]
  if (!file.exists(paste0("./out/",SV,"/",SV,"_chr_1with_add_dom.rds"))){cat(SV, "does not exist!\n") 
    next}
  taxa <- pheno[SV]
  ...
  lmm.data <- merge(taxa, pheno, by="row.names")
  ...
  for (chr in 1:19){
    out <- readRDS(paste0("./out/",SV,"/",SV,"_chr_",chr,"with_add_dom.rds"))
    ...
    system.time(f<-mclapply(sub, function(snp){
      ...
      model <- relmatLmer(tax ~ ad+dom + (1|id), df,relmat=list(id=kinship))
      res<-c(snp,nrow(df),table(factor(df$ad,levels=c(-1,0,1))),tryCatch(summary(model)$coefficients[2,], error=function(x) return(rep(NA,3))),tryCatch(summary(model)$coefficients[3,], error=function(x) return(rep(NA,3))),tryCatch(anova(null_model,model)[2,8], error=function(x) return(NA)))
      names(res)<-c("marker","n","AA","AB","BB","add.Beta","add.StdErr","add.T","dom.Beta", "dom.StdErr", "dom.T", "P")
      #out[snp,8:18]<-res[2:12]
      return(res)},mc.cores=getOption("mc.cores", 20)))
    f2 <-data.frame(do.call(rbind, f),stringsAsFactors = F)
    out[out$marker %in% f2$marker,8:18] <- data.frame(lapply(f2[,2:12],as.numeric))
    gwasResults <- rbind(gwasResults, out)
    saveRDS(out,paste0("out/",tax,"/",tax,"_chr_",chr,"_new.rds"))
    head(out)
  }
  saveRDS(gwasResults,paste0("out/",tax,"/",tax,"_new.rds"))
