  ...
  trait_list <- read_csv2("~/Documents/PhD/Experiments/Final_QTL_mapping/Scripts/Summary_tables/trait.list.dna.csv")
  ...
  for(i in 2:ncol(trait_list)){
    trait = colnames(trait_list)[i]
    dir.create(trait, showWarnings = FALSE)
    tax_list = na.omit(trait_list[,i])
  ...
  save(list=paste(gws,"_gwscan",sep=""),file=paste(trait, "/",gws,"_gwscan_allchr.Rdata",sep=""))
...
#initialise at 0 for those with none sig
  ...
  save(list=paste(gws,"_gwscan_sigSNPs",sep=""),file=paste(trait, "/",gws,"_gwscan_sigSNPs.Rdata",sep=""))
...
for(P.type in c("P","add.P","dom.P")){
  if(length(which(gwscan_sig[,paste0(P.type,".sig.",sig.type)]=="Y"))>0){
    ...
    save(list=paste(gws,"_gwscan_sigIntervals.",P.type,".",sig.type,sep=""),file=paste(trait, "/",gws,"_gwscan_sigIntervals.",P.type,".",sig.type,".Rdata",sep=""))
}
}
