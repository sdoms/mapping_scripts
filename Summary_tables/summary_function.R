summary_tables_histo <-
  function(trait,
    ...
    write.table(x = unique(sig.P),
                file = paste0(output, "_all_sig_snps_P_", method,".txt"))
    write.table(x = unique(sig.add.P),
                file = paste0(output, "_all_sig_snps_add_P_", method,".txt"))
    write.table(x = unique(sig.dom.P),
                file = paste0(output, "_all_sig_snps_dom_P_", method,".txt"))
    ...
    ggsave(paste0(output, "_all_sig_snps-zscore_", method,".pdf"))
    ...
    write.table(x = tt,
                file = paste0(output, "_all_unique_sig_snps_ALL_P_", method,".txt"))
    ...
    write.csv(final_out, paste0(output, "_markers_with_genes_", method,".csv"), row.names = F)
