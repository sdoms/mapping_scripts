  ...
  trait_list <- read_csv("~/Dropbox/BainesTurnerShared2.0/Results/Bacula/trait.list.csv")
  Xtrue = T
  #folder that has a results folder for each trait
  results.folder="/Users/turner/Dropbox/BainesTurnerShared2.0/Results/Bacula/"
  
  #Fix bacula trait names with covars
  ...
  if (Xtrue){
    ...
    if(which(colnames(X_chrom)=="trait")!=0) X_chrom=X_chrom[,-which(colnames(X_chrom)=="trait")]
    #X_chrom$chr <- "20"
...
for(P.type in c("P","add.P","dom.P")){
  if(length(which(gwscan_sig[,paste0(P.type,".sig.",sig.type)]=="Y"))>0){
    ...
    sig.bins=data.frame(chr_bin=unique(gwscan_sigsnp_bins$chr_bin),chr.num=0,start.snp="a",start.pos=0,peak.snp="a",peak.P.type=0,peak.pos=0,stop.snp="a",stop.pos=0,sig.snps=0)
    for(k in 1:nrow(sig.bins)){
      ...
      sig.bins$peak.P.type[k]=snps.sub[which.min(snps.sub[,P.type]),P.type]
      }
    ...
    if(nrow(sig.bins)>1){
      ...
      if(length(redo.bin)>0){
        chr.redo=unique(sig.bins[redo.bin,]$chr.num)
          ...
          sig.bins.chr =data.frame(chr_bin=unique(sig.bins.chr.old$chr_bin.new),chr.num=chr,start.snp="a",start.pos=0,peak.snp="a",peak.P.type=0,peak.pos=0,stop.snp="a",stop.pos=0,sig.snps=0,start.LD.pos=0,stop.LD.pos=0)
            for(k in 1:nrow(sig.bins.chr)){
              ...
              peak.row=which.min(sig.bins.chr.old[sig.bins.chr.old$chr_bin.new==sig.bins.chr$chr_bin[k],]$peak.P.type)
              sig.bins.chr$peak.snp[k]=sig.bins.chr.old$peak.snp[peak.row]
              sig.bins.chr$peak.P.type[k]=sig.bins.chr.old$peak.P.type[peak.row]
...
rownames(sig.intervals.comb)=1:nrow(sig.intervals.comb)
  ...
  keep.cols=c("trait","P.type","sig.type","chr","chr.num","chr_bin","start.LD.pos","stop.LD.pos","sig.snps","peak.snp","rsId","peak.pos","peak.P.type","add.Beta_peak_snp","add.StdErr_peak_snp","add.Zscore_peak_snp","add.P_peak_snp","dom.Beta_peak_snp","dom.StdErr_peak_snp","dom.Zscore_peak_snp","dom.P_peak_snp","markers","closest_gene","closest_gene_type","closest_gene_description","total_genes","list_total_genes","total_protein_coding_genes","list_protein_coding_genes")
  sig.intervals.out=sig.intervals.out[,keep.cols]
  #rename some columns
  colnames(sig.intervals.out[,"start.LD.pos"])="start"
