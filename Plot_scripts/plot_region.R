...
plot.region <- function(trait,tax_level, chr, start, stop, peak_snp,P="P",dnaorrna, outputdir=".", gene.ensembl){
  ...
  ss <- GM_snps  %>% dplyr::filter(between(pos*1e6, start, stop))
  max_ld <- merge(ss, ld_set, by.x="marker", by.y="SNP_B")
  
  para.region <- gwscan %>% filter(between(pos*1e6, start, stop))
  ...
  p1 <- ggplot() + 
    geom_point(data = para.region2,aes(pos, log10p,shape=highlight, colour = R2, fill=highlight), size=3, stroke=1) + 
...
p2
  p1b <- p1 + xlab("") + theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank()) + 
    xlim(start,stop) +geom_vline(xintercept = begin, linetype="dotted") + geom_vline(xintercept = end, linetype="dotted") +
