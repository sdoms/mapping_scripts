setwd("/Users/doms/Documents/PhD/Experiments/Final_QTL_mapping/Results/Bacterial traits/RNA/sig.summaries")
load("../../../../Cleaning_snps/clean_snps.Rdata")

load("/Users/doms/Documents/PhD/Experiments/Final_QTL_mapping/Results/Bacterial traits/Genes/genes_overlap/SW/genes_DNA_RNA_intervals_SW.Rdata")
...
# median 
input_file_DNA<- transform(input_file_DNA, length_num=as.numeric(length))
input_file_RNA<- transform(input_file_RNA, length_num=as.numeric(length))
input_file<- transform(input_file, length_num=as.numeric(length))


med_DNA<- median(input_file_DNA$length_num[input_file_DNA$length_num>0], na.rm=T)
med_RNA<- median(input_file_RNA$length_num[input_file_RNA$length_num>0], na.rm=T)
med_tot <-median(input_file$length_num[input_file$length_num>0], na.rm=T)
#start plot
output <- data.frame()
for (chr in goodChrOrder){
  df.tmp <- input_file[input_file$chr.num==chr,] %>% drop_na(start.LD.pos)
  dd<- c()
...
ggsave("intervals_RNA_plot_GW.pdf")
ggsave("intervals_DNA_RNA_plot_GW.pdf")
# study-wide
SW_threshold =0.05/32625/118.2177
input_P <- input_file[input_file$P.type=="P"&input_file$peak.P.type<SW_threshold,]
input_add<- input_file[input_file$P.type=="add.P"&input_file$add.P_peak_snp<SW_threshold,]
input_dom <- input_file[input_file$P.type=="dom.P"&input_file$dom.P_peak_snp<SW_threshold,]
input_SW<- rbind(input_P, input_add)
input_SW <- rbind(input_SW, input_dom)
...
med_DNA_SW<- median(input_SW$length_num[input_SW$length_num>0&input_SW$DNAorRNA=="DNA"], na.rm=T)
med_RNA_SW<- median(input_SW$length_num[input_SW$length_num>0&input_SW$DNAorRNA=="RNA"], na.rm=T)
med_tot_SW<- median(input_SW$length_num[input_SW$length_num>0], na.rm=T)

output <- data.frame()
for (chr in goodChrOrder){
  df.tmp <- input_SW[input_SW$chr.num==chr,] %>% drop_na(start.LD.pos)
  if(nrow(df.tmp!=0)){
...
snp_plot <- ggplot()+
  ...
  labs(y="Position (Mb)", x= "Chromosome", color="Overlap count") +theme_test()
snp_plot + scale_x_discrete(limits=factor(goodChrOrder))
ggsave("intervals_RNA_plot_SW.pdf")
write_csv2(input_SW, file="results_RNA_SW.csv")
ggsave("intervals_DNA_RNA_plot_SW.pdf")
write_csv2(input_SW, file="results_DNA_RNA_SW.csv")

median_interval_size <- median(input_file$length)
...
unique_int_GW <- input_file %>% 
  group_by(chr) %>% 
  distinct(start.LD.pos, stop.LD.pos)
un_marker_GW<- unique(input_file$peak.snp)
