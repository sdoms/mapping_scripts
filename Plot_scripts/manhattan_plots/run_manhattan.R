...
#### gapped ###
p1 <-gg.manhattan(gwscan = taxa_X, SNP="marker", CHR = "chr", BP="pos", P="P",threshold=threshold, col = mypalette, title = "", method="bon")+theme_test()

p2<- gg.manhattan(taxa,SNP="marker", CHR = "chr", BP="pos", P="add.P",threshold=threshold, col = mypalette, title = "",method="bon") +theme_test()


p3<-gg.manhattan(taxa,SNP="marker", CHR = "chr", BP="pos", P="dom.P", threshold=threshold,col = mypalette, title ="", method="bon") +theme_test()
p <- p1 /(p2+p3)
p1b<-gg.gap(p1, segments=list(c(18,25),c(27,39), c(41,62)), ylim = c(0,70), tick_width = c(1,1,1,2), rel_heights = c(1,0,0.2,0,0.2,0,0.2)) 
p1b
ggsave("manhattan_SV184_P.pdf",width = 7, height = 8)
p2b<-gg.gap(p2, segments=list(c(20,31),c(32,55), c(56,112)), ylim = c(0,120), tick_width = c(1,1,1,2), rel_heights = c(1,0,0.1,0,0.1,0,0.2)) 
ggsave("manhattan_SV184_addP.pdf",width = 7, height = 8, plot=p2b)
p3b<-gg.gap(p3, segments=list(c(22,28),c(30,53), c(55,104)), ylim = c(0,115), tick_width = c(1,1,1,2), rel_heights = c(1,0,0.2,0,0.2,0,0.2)) 
p3b
ggsave("manhattan_SV184_domP.pdf",width = 12, height = 15, plot=p3b)
pb <- p1b /(p2b+p3b) +plot_annotation(tag_levels = "A")

ggsave("all_SV184.pdf", plot=pb,width = 12, height = 15)
