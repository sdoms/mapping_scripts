...
chrSizes<-as.data.frame(seqlengths(TxDb.Mmusculus.UCSC.mm10.knownGene)) %>% rownames_to_column("chr") 
colnames(chrSizes)<- c("chr", "chr_len")
chrSizes<- chrSizes[1:20,]
chrSizes$chr_num<-c(1:19,"X")
...
inputfile <- read.csv("all_markers_RNA_DNA-with-genes_SW_pval_corr.csv", sep=",")
...
output <- inputfile %>% dplyr::select(chr, pos, total_count) %>% mutate(chr=as.character(chr),start=pos, end=pos+0.5)
...
snp_plot <- ggplot()+
  geom_segment(data = chrSizes,
               aes(x = chr_num, xend = chr_num, y = 0, yend = chr_len),
               lineend = "round", color = "#e0e7eb", size = 5) + 
  
  geom_segment(data=output, 
               aes(x=chr, xend=chr, y=start, yend=end, color=total_count), size=5)+
  ...
  scale_x_discrete(limits=goodChrOrder)+ theme(text = element_text(size=12))+
  scale_y_continuous(labels = scales::unit_format(unit = "Mb", scale = 1e-6))
snp_plot
ggsave("SNP_density_results_SW_pval_corr.pdf")
save(snp_plot, file="snp_plot_pval_corr.Rdata")
library(patchwork)
...
#   
# }
# save(output, file="overlap_all_results_pval_corr_SW.Rdata")
...
#   
# }
# write_delim(top50, file="top50_snps_pval_corr_SW.csv", delim=";")
