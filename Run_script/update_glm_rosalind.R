...
pheno <- read.csv("../psem/input/Phenotypes_with_time.csv", sep=";", header=T)
rownames(pheno)<-pheno$id

taxa <-read.csv(paste0("../psem/input/",trait,"_table_f2_core.csv"), header=T, row.names = 1)
...
geno<- BEDMatrix("../psem/input/clean_f2")
...
load("../psem/input/clean_snps.Rdata")
...
for (chr in 1:19){
  ...
  if (DNAorRNA=="DNA"){
    kinship <- read.table(paste0("../psem/input/kinship_DNA/kinship_chr",chr,".cXX.txt"))
  }else if (DNAorRNA=="RNA"){
    kinship <- read.table(paste0("../psem/input/kinship_RNA/kinship_chr",chr,".cXX.txt"))
  }
  ...
  saveRDS(out,paste0("./output/",DNAorRNA,"/",trait,"/",tax,"_chr_",chr,"_with_add_dom_P.rds"))
  head(out)
  # }
...
saveRDS(gwasResults,paste0("./output/",DNAorRNA,"/",trait,"/",tax, "_with_add_dom_P.rds"))
